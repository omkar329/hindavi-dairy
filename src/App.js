...
export default function HindaviDairy() {
  ...
  // ====================================================
  // 📥 PDF REPORT DOWNLOAD - jsPDF Direct Download
  // ====================================================
  const downloadPDFReport = (farmerId) => {
    ...
    const totalLitres = records.reduce((acc, r) => acc + (parseFloat(r.litres) || 0), 0);
    const totalAmount = records.reduce((acc, r) => acc + (parseFloat(r.amount) || 0), 0);
    const avgFat = (records.reduce((acc, r) => acc + (parseFloat(r.fat) || 0), 0) / records.length).toFixed(2);
    const avgSMF = (records.reduce((acc, r) => acc + (parseFloat(r.smf) || 0), 0) / records.length).toFixed(2);

    const doc2 = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
    const pw = doc2.internal.pageSize.getWidth();

    // === HEADER BACKGROUND ===
    doc2.setFillColor(15, 32, 39);
    doc2.rect(0, 0, pw, 38, "F");

    // === DAIRY NAME ===
    doc2.setFont("helvetica", "bold");
    doc2.setFontSize(18);
    doc2.setTextColor(247, 183, 49);
    doc2.text("Hindavi Dudha Sankalan Kendra", pw / 2, 14, { align: "center" });

    doc2.setFontSize(11);
    doc2.setTextColor(170, 212, 245);
    doc2.text("Chincholi", pw / 2, 21, { align: "center" });

    // gold divider line
    doc2.setDrawColor(247, 183, 49);
    doc2.setLineWidth(0.7);
    doc2.line(14, 26, pw - 14, 26);

    doc2.setFontSize(10);
    doc2.setTextColor(255, 255, 255);
    doc2.text("10-Diwasiya Dudha Sankalan Ahwal", pw / 2, 33, { align: "center" });

    // === FARMER NAME BOX ===
    doc2.setFillColor(255, 248, 225);
    doc2.roundedRect(14, 42, pw - 28, 22, 3, 3, "F");
    doc2.setDrawColor(247, 183, 49);
    doc2.setLineWidth(1);
    doc2.roundedRect(14, 42, pw - 28, 22, 3, 3, "S");

    doc2.setFont("helvetica", "bold");
    doc2.setFontSize(13);
    doc2.setTextColor(15, 32, 39);
    doc2.text(`Shetkari: ${farmer?.name || '-'}`, pw / 2, 51, { align: "center" });

    doc2.setFont("helvetica", "normal");
    doc2.setFontSize(9);
    doc2.setTextColor(80, 80, 80);
    doc2.text(`Gav: ${farmer?.village || '-'}   |   Mobile: ${farmer?.mobile || '-'}   |   Date: ${new Date().toLocaleDateString('en-IN')}`, pw / 2, 59, { align: "center" });

    // === TABLE ===
    const tableData = records.map(r => [
      r.date,
      r.session,
      parseFloat(r.litres).toFixed(2),
      parseFloat(r.fat || 0).toFixed(2),
      parseFloat(r.smf || 0).toFixed(2),
      "Rs." + parseFloat(r.rate).toFixed(2),
      "Rs." + r.amount
    ]);
    ...
    autoTable(doc2, {
      startY: 68,
      head: [["Tarikh", "Satr", "Dudha (Li.)", "Fat (%)", "SMF (%)", "Dar (Rs.)", "Ekun Rakam"]],
      body: tableData,
      theme: "grid",
      headStyles: {
        fillColor: [32, 58, 67],
        textColor: [247, 183, 49],
        fontStyle: "bold",
        halign: "center",
        fontSize: 10
      },
      ...
      columnStyles: {
        ...
        6: { textColor: [26, 122, 63], fontStyle: "bold" }
      },
      ...
      didParseCell: (data) => {
        if (data.row.index === tableData.length - 1) {
          ...
          data.cell.styles.textColor = [15, 32, 39];
        }
      }
    });

    const finalY = doc2.lastAutoTable.finalY + 10;

    // === SUMMARY BOXES ===
    doc2.setFillColor(15, 32, 39);
    doc2.roundedRect(14, finalY, pw - 28, 28, 3, 3, "F");

    const boxes = [
      { label: "Ekun Dudha", value: totalLitres.toFixed(2) + " Li." },
      { label: "Sarasari Fat", value: avgFat + "%" },
      { label: "Sarasari SMF", value: avgSMF + "%" },
      { label: "Ekun Rakam", value: "Rs." + totalAmount }
    ];
    const boxW = (pw - 28) / 4;
    boxes.forEach((b, i) => {
      const x = 14 + i * boxW + boxW / 2;
      doc2.setFont("helvetica", "normal");
      doc2.setFontSize(8);
      doc2.setTextColor(170, 212, 245);
      doc2.text(b.label, x, finalY + 10, { align: "center" });
      doc2.setFont("helvetica", "bold");
      doc2.setFontSize(12);
      doc2.setTextColor(247, 183, 49);
      doc2.text(b.value, x, finalY + 22, { align: "center" });
    });
    ...
    doc2.text(
      "Hindavi Dudha Sankalan Kendra, Chincholi | Sanganak Pranali dwara tayar | Dhanyavad!",
      pw / 2, finalY + 40, { align: "center" }
    );

    // === DOWNLOAD ===
    doc2.save(`Hindavi_Report_${farmer?.name || 'Shetkari'}_${new Date().toLocaleDateString('en-IN').replace(/\//g,'-')}.pdf`);
    showNotif("PDF यशस्वीरित्या डाऊनलोड झाली! ✅");

  };

  // ====================================================
  // (Old CSV kept as fallback - can be removed)
              ...
              {selectedFarmerReport && (
                <div>
                  <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
                    {/* 🆕 PDF DOWNLOAD BUTTON */}
                    <button onClick={() => downloadPDFReport(selectedFarmerReport)} style={S.btn("#e74c3c")}>📄 PDF रिपोर्ट डाऊनलोड</button>
                    <button onClick={() => downloadCSVReport(selectedFarmerReport)} style={S.btn("#3498db")}>📥 CSV डाऊनलोड</button>
    ...
    return (
      <div style={S.app}>
        ...
        <div style={{ padding: 16 }}>
          ...
          {/* PDF REPORT DOWNLOAD FOR USER */}
          <div style={S.card}>
            <h3 style={{ color: "#f7b731", marginTop: 0 }}>📊 तुमचा १०-दिवसांचा रिपोर्ट</h3>
            <p style={{ fontSize: 13, color: "#e0e0e0", marginBottom: 12 }}>
              तुमच्या शेवटच्या १० दिवसांच्या दूध संकलनाचा गोषवारा PDF/Print स्वरुपात डाऊनलोड करा.
            </p>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {/* 🆕 PDF BUTTON FOR USER TOO */}
              <button 
                onClick={() => downloadPDFReport(currentUser.id)} 
                style={S.btn("#e74c3c")}
              ...
              userRecords.map(c => (
                <div key={c.firebaseId || c.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.1)", padding: "12px 0" }}>
                  ...
                  <div style={{ fontSize: 14, color: "#e0e0e0", margin: "4px 0" }}>
                    दूध: <b>{c.litres} लि.</b> | फॅट: {c.fat}% | SMF: {c.smf}% | दर: ₹{c.rate}
                  </div>
